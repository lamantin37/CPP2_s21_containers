#ifndef CPP2_S21_CONTAINERS_S21_BASE_SET_H_
#define CPP2_S21_CONTAINERS_S21_BASE_SET_H_
...
namespace s21 {
template <typename Key, bool AllowsDuplicates>
class s21_base_set {
 public:
  using key_type = Key;
  ...
  s21_base_set() : rb_tree_(){};

  s21_base_set(std::initializer_list<value_type> const& items) {
    for (const auto& item : items) rb_tree_.insertValue(AllowsDuplicates, item);
  }

  s21_base_set(const s21_base_set& s) { copy(s.rb_tree_.getNode()); }

  s21_base_set& operator=(const s21_base_set& s) {
    return *this = s21_base_set(s);
  }

  s21_base_set(s21_base_set&& s) noexcept : rb_tree_(std::move(s.rb_tree_)) {
    s.clear();
  }

  s21_base_set& operator=(s21_base_set&& s) {
    if (this != &s) {
      clear();
  ...
  std::pair<iterator, bool> insert(const value_type& value) {
    ...
    bool inserted = !node_found || AllowsDuplicates;
    if (inserted) rb_tree_.insertValue(AllowsDuplicates, value);
    return {
  ...
  void swap(s21_base_set& other) noexcept {
    std::swap(rb_tree_, other.rb_tree_);
  }

  void merge(s21_base_set& other) {
    for (iterator it = other.begin(); it != other.end(); ++it) {
      rb_tree_.insertValue(AllowsDuplicates, *it);
    }
    other.clear();
  }
  // ========================================================
  ...
  std::pair<iterator, bool> insert_many(Args&&... args) {
    ...
    rb_tree_.insertValue(AllowsDuplicates, value);
    return {iterator(rb_tree_.findValue(value), &rb_tree_), true};
  }
  // ========================================================
 ...
 protected:
  ...
  void copy(node<value_type>* n) {
    if (n && !rb_tree_.findValue(n->key_value.first)) {
      rb_tree_.insertValue(AllowsDuplicates, n->key_value.first);
      copy(n->left);
      copy(n->right);
    }
  }
...
#endif  // CPP2_S21_CONTAINERS_S21_BASE_SET_H_
