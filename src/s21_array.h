...
class array {
 public:
    ...
    array(array&& other) : _data(nullptr), size_(0), capacity_(0) {
        swap(other);
    }
    ...
    reference at(size_type pos) {
        return (pos >= size_ ? (throw std::out_of_range("Index out of range")) : _data[pos]);
    }
    ...
    const_reference back() const {
        return empty() ? (throw std::out_of_range("Array is empty")) : _data[size_ - 1];
    }
    // iterator data() {}
